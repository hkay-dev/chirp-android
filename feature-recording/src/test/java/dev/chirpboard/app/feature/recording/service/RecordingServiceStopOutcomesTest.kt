...
class RecordingServiceStopOutcomesTest {
    ...
    fun setup() {
        ...
        coEvery { recordingRepository.deleteInProgressRecording(any()) } just runs
    ...
    @Test
    fun noAudioFile_deletesInProgressRowAndCompletes() = runTest {
        ...
        coVerify(exactly = 1) { recordingRepository.deleteInProgressRecording(recordingId) }
    ...
    fun staleGeneration_discardsPersistResultWithoutStateTransition() = runTest {
        ...
        verify(exactly = 0) { recordingStateManager.onRecordingError(any(), any()) }
        coVerify(exactly = 0) { recordingRepository.deleteInProgressRecording(any()) }
        verify {
