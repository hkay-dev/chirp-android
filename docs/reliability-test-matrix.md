...
| Quick-input focus handoff | Recognition activity hides the caller IME, joins Chirp's main task, or triggers an extra IME-control transition | merged-manifest verification, `RecognitionDeliveryTest`, repeated Telegram and Twitter device checks | `:app:testDebugUnitTest`, `:app:processBetaManifest` |
