...
| Quick-input focus and result handoff | Recognition activity hides the caller IME, drops a caller-supplied result `PendingIntent`, double-delivers, or leaves X with stale visible editor state | merged-manifest verification, `RecognitionDeliveryTest`, `RecognitionActivityResultContractTest`, repeated Telegram and X device checks | `:app:testDebugUnitTest`, `:app:compileDebugAndroidTestKotlin`, `:app:processBetaManifest` |
