...
| Quick-input focus and result handoff | Recognition activity hides the caller IME, drops a caller-supplied result `PendingIntent`, double-delivers, manipulates another app through accessibility, loses transcription on activity teardown, or dismisses the copy fallback on tap | merged-manifest verification, `RecognitionDeliveryTest`, `RecognitionActivityResultContractTest`, `DictationCapturePersistenceGuardTest`, `VoiceRecognitionTranscriptionRunnerTest`, `QuickInputResultNotificationTest`, repeated Telegram, X, and Gemini device checks, stock-Google control run | `:app:testDebugUnitTest`, `:feature-transcription:testDebugUnitTest`, `:app:compileDebugAndroidTestKotlin`, `:app:processBetaManifest` |
