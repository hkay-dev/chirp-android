...
| Quick-input focus and result handoff | Recognition activity hides the caller IME, drops a caller-supplied result `PendingIntent`, double-delivers, changes a host field automatically, overwrites a changed field, or leaves no paste/copy fallback when SwiftKey loses its `InputConnection` | merged-manifest verification, `RecognitionDeliveryTest`, `RecognitionActivityResultContractTest`, `DictationCapturePersistenceGuardTest`, `QuickInputResultNotificationTest`, `QuickInputAccessibilityCoordinatorTest`, repeated Telegram, X, and Gemini device checks, stock-Google control run | `:app:testDebugUnitTest`, `:feature-transcription:testDebugUnitTest`, `:app:compileDebugAndroidTestKotlin`, `:app:processBetaManifest` |
