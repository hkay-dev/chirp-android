...
| Quick-input focus and result handoff | Recognition activity hides the caller IME, drops a caller-supplied result `PendingIntent`, double-delivers, or leaves SwiftKey's result pending when X drops its inline reply `InputConnection` | merged accessibility-service boundary, `RecognitionDeliveryTest`, `RecognitionActivityResultContractTest`, `QuickInputFocusRecoveryCoordinatorTest`, repeated Telegram and X device checks, stock-Google control run | `:app:testBetaUnitTest`, `:app:compileDebugAndroidTestKotlin`, `:app:processBetaManifest` |
