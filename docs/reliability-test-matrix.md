...
| Quick-input focus handoff | Recognition activity hides the caller IME or delivers success while it still owns key-window focus | merged-manifest verification, `RecognitionDeliveryTest`, repeated Telegram and Twitter device checks with no IME-control timeout | `:app:testDebugUnitTest`, `:app:processBetaManifest` |
