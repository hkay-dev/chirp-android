...
| Quick-input focus and result handoff | Recognition activity hides the caller IME, drops a caller-supplied result `PendingIntent`, double-delivers, manipulates another app through accessibility, or leaves no copy fallback when X drops its inline reply `InputConnection` before SwiftKey's deferred commit | merged-manifest verification, `RecognitionDeliveryTest`, `RecognitionActivityResultContractTest`, `DictationCapturePersistenceGuardTest`, `QuickInputResultNotificationTest`, repeated Telegram, X, and Gemini device checks, stock-Google control run | `:app:testDebugUnitTest`, `:feature-transcription:testDebugUnitTest`, `:app:compileDebugAndroidTestKotlin`, `:app:processBetaManifest` |
