...
| Quick-input focus and result handoff | Recognition activity hides the caller IME, drops a caller-supplied result `PendingIntent`, double-delivers, or loses text when X drops its inline reply `InputConnection` before SwiftKey's deferred commit | merged-manifest verification, `RecognitionDeliveryTest`, `RecognitionActivityResultContractTest`, repeated Telegram and X device checks, stock-Google control run | `:app:testDebugUnitTest`, `:app:compileDebugAndroidTestKotlin`, `:app:processBetaManifest` |
