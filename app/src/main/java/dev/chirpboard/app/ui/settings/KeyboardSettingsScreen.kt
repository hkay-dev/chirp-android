...
import dev.chirpboard.app.quickinput.QuickInputAccessibilityService
...
fun KeyboardSettingsScreen(
    ...
    var isReliablePasteEnabled by remember { mutableStateOf(false) }
    LifecycleResumeEffect(Unit) {
        isKeyboardEnabled = isChirpKeyboardEnabled(context)
        isReliablePasteEnabled = isQuickInputAccessibilityEnabled(context)
        onPauseOrDispose { }
    }
        ...
        LazyColumn(
            ...
            item {
                SettingsListItem(
                    ...
                    title = stringResource(R.string.keyboard_settings_reliable_paste_title),
                    subtitle =
                        if (isReliablePasteEnabled) {
                            stringResource(R.string.keyboard_settings_reliable_paste_description_enabled)
                        } else {
                            stringResource(R.string.keyboard_settings_reliable_paste_description)
                        },
                    badge = if (isReliablePasteEnabled) SettingsBadge.CONNECTED else null,
                    onClick = {
                        try {
...
/** Returns whether the user enabled Chirp's package-limited tap-to-paste service. */
internal fun isQuickInputAccessibilityEnabled(context: Context): Boolean =
    try {
        val manager = context.getSystemService(AccessibilityManager::class.java) ?: return false
        val expected = ComponentName(context, QuickInputAccessibilityService::class.java)
        manager
