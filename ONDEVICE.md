...
- [ ] **KB-7 Global floating mic.** Enable Chirp's accessibility service, then open a normal text field with Gboard plus SwiftKey, followed by Samsung Keyboard. The bubble appears once above each keyboard. Drag it to both edges and several heights, close and reopen the keyboard, then rotate; the bubble returns once at the saved edge and relative height. Tap it and confirm the editor switch starts off plus the labeled Cancel button stays visible. Dictate and stop; the preferred result is copied automatically, no result notification appears, and the sheet closes. Repeat with the editor switch on. Confirm the editable review opens in the recognition sheet, every configured AI preset can replace the draft, and an AI failure keeps the draft. Confirm emoji plus multiline edits survive exactly, Copy and close replaces the clipboard text, and the original field stays unchanged. Clear the draft and check Copy and close is disabled. Close and Back keep the automatically copied result. Rotate with an edited review open and confirm the draft plus selection survive without starting another recording. Check a password field hides the bubble. Disable the accessibility service and confirm the bubble disappears at once.
