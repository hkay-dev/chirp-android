...
class ProfileRepositoryTest {
    ...
    @Test
    fun `getAllProfilesList returns list from dao`() = runTest {
        val expected = listOf(Profile(id = UUID.randomUUID(), name = "Test"))
    ...
    @Test
    fun `getProfileFlow returns flow from dao`() = runTest {
        val id = UUID.randomUUID()
        val expected = Profile(id = id, name = "Test Profile")
    ...
    @Test
    fun `createProfile inserts and returns new profile`() = runTest {
        coEvery { profileDao.getMaxSortOrder() } returns 5
        val result = repository.createProfile(name = "New Profile")
