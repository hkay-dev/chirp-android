...
class RecordingRepository
    ...
    constructor(
        ...
        fun getAllRecordings(): Flow<List<Recording>> =
            recordingDao.getAllRecordings().catch {
        ...
        fun getRecordingFlow(id: UUID): Flow<Recording?> =
            recordingDao.getRecordingFlow(id).catch {
                if (it is CancellationException) throw it
                emit(null)
            }

        fun getRecordingsByStatus(status: RecordingStatus): Flow<List<Recording>> =
            recordingDao.getRecordingsByStatus(status).catch {
                if (it is CancellationException) throw it
                emit(emptyList())
            }
        ...
        fun searchRecordings(query: String): Flow<List<Recording>> =
            recordingDao.searchRecordings(query).catch {
                if (it is CancellationException) throw it
                emit(emptyList())
            }
        ...
        fun getTranscriptFlow(recordingId: UUID): Flow<Transcript?> =
            transcriptDao.getTranscriptFlow(recordingId).catch {
                if (it is CancellationException) throw it
                emit(null)
            }
